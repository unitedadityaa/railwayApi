...
router.patch("/update-llm", async (req, res) => {
    try {
        const { userId, llmId, businessName, timezone, contactMethod } = req.body;

        // ✅ Validate Inputs
        if (!userId || !llmId || !businessName || !timezone || !contactMethod) {
        ...
        const updatedPrompt = generateCustomPrompt({
            businessName,
            agentName: "Sia", // Assume agent name is set already
            contactMethod,
            currentTime,
        });
